...
function RGBtoHEX(rgb) {
  // [R1,R2,G1,G2,B1,B2]
  var hexValues = [Math.floor(rgb[0]/16), rgb[0]%16,  Math.floor(rgb[1]/16), rgb[1]%16, Math.floor(rgb[2]/16), rgb[2]%16];
  var hexString = '#';
...
function linearlyChangeRGB(rgbEnd, animTime) {
  ...
  */
  ...
  var rgbStartArray = rgbStartString.substring(4, rgbStartString.length - 1).split(",");  // splits the string into an array of RGB values
  var rgbStart = [parseInt(rgbStartArray[0]),parseInt(rgbStartArray[1]),parseInt(rgbStartArray[2])];
  // Calculate necessary information
  var rgbDiff = [rgbEnd[0]-rgbStart[0], rgbEnd[1]-rgbStart[1], rgbEnd[2]-rgbStart[2]];
  ...
  let animInterval = setInterval(() => {
    ...
    stepCtr++;
    $("#logo-svg-V").attr("fill", `rgb(${rgbCurrent[0]},${rgbCurrent[1]},${rgbCurrent[2]})`);
    $("#movingbar").css("background-color", `rgb(${rgbCurrent[0]},${rgbCurrent[1]},${rgbCurrent[2]})`);
  }, millisecsInterval);
}
...
------------------------------------------------------------------------------- */
function expandSelectionBox(element, strHeader, strText, expWidth, animTime, colorScheme, initialFlexBasis) {
  /*
  ...
  */
  ...
  if((parseInt(element.css("width")) > 20) && ($(window).width() / $(window).height() > 1.7)) {
    ...
    element.append(`<div class="selectionBoxDiv"><p class="selectionBoxHeader">${strHeader}</p><p class="selectionBoxText">${strText}</p></div>`);
...
function contractSelectionBox(element, animTime, colorScheme, initialFlexBasis) {
  /*
  Params:   element:         jQuery-object of the selection box' div, e.g. "$('#selectionDiv')".
                              IMPORTANT: HTML of element needs to be structured like:
                              <div>...<svg>...<g>...<path>... with only ONE <g>! 
  ...
  if(element.find('.selectionBoxDiv').length) {     
    ...
    $('.selectionBoxDiv').animate({
      ...
      complete: () => {
        ...
        element.animate({
          "flex-basis": `-=${parseInt(element.css("flex-basis"))-initialFlexBasis}`
        }, {
          duration: animTime,
          easing: "swing",
          complete: () => {
...
function hoverMenupoint(elementNo, animTime, colorScheme, reverseFlag=false) {
  ...
  */
  ...
  if(!reverseFlag) {
    ...
    element.css("cursor","pointer");
    element.css("color", `rgb(${colorScheme[elementNo+2][0]},${colorScheme[elementNo+2][1]},${colorScheme[elementNo+2][2]})`);
    ...
    $(':animated').stop(true);                // Stop ongoing anims
    // Change element cursor and color back to black
      // Change cursor type and color of element
    element.css("cursor","default");
    element.css("color", `rgb(${colorScheme[1][0]},${colorScheme[1][1]},${colorScheme[1][2]})`);
...
function hoverExplore(animTime, colorScheme, reverseFlag=false) {
  if(!reverseFlag) {
...
function clickThemeChange(element, animTime, colorsJSON, colorTheme, initialFlexBasis) {
  ...
  */
  ...
  $('#h2menu0').css("color", `rgb(${colors[1][0]},${colors[1][1]},${colors[1][2]})`);
  $('#h2menu1').css("color", `rgb(${colors[1][0]},${colors[1][1]},${colors[1][2]})`);
  $('#h2menu2').css("color", `rgb(${colors[1][0]},${colors[1][1]},${colors[1][2]})`);
  $('#h2menu3').css("color", `rgb(${colors[1][0]},${colors[1][1]},${colors[1][2]})`);
  $('#exploreText').css("color", `rgb(${colors[1][0]},${colors[1][1]},${colors[1][2]})`);
  $('#exploreArrowPolygon').css("fill", `rgb(${colors[1][0]},${colors[1][1]},${colors[1][2]})`);
  $('#exploreArrowPolygon').css("stroke", `rgb(${colors[1][0]},${colors[1][1]},${colors[1][2]})`);
